    ...
    <div class="content">
      ...
      <div class="d-md-flex d-block align-items-center justify-content-between mb-3">
        ...
        <top-action-buttons
          ...
          @export="exportData"
          @addRole="openAddRoleModal"
        />
      </div>

      <!-- Using the alert component -->
      <alert v-if="notificationStore.notification" :type="notificationStore.notification.status === 'error' ? 'danger' : notificationStore.notification.status" :icon="notificationStore.notification.status === 'error' ? 'alert-octagon' : 'check-circle'" :message="notificationStore.notification.message" @update:message="notificationStore.notification.message = $event" />

      <card
        title="Roles & Permissions List"
        v-model:dateRange="dateRange"
        v-model:currentSort="currentSort"
        @update:currentSort="sortRoles"
