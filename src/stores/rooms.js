import { defineStore } from 'pinia'
import axios from 'axios'

export const useRoomsStore = defineStore('rooms', {
  state: () => ({
    rooms: [],
    id: null,
    currentPage: 1,
    perPage: 10,
    total: 0,
    search: '',
    status: null,
    sort: 'asc',
    dateStart: '',
    dateEnd: '',
    exportType: 'pdf',
  }),

  actions: {
    /* -------------------------------------------------------------------
     * FETCH ROOMS
     * ------------------------------------------------------------------- */
    async index() {
      try {
        const params = {
          date_start: this.dateStart,
          date_end: this.dateEnd,
          per_page: this.perPage,
          page: this.currentPage,
          search: this.search?.trim() || null,
          sort: this.sort,
          status: this.status
        }
        const response = await axios.get('/rooms', { params })

        if (response?.status === 200) {
          this.rooms = response.data.data || [];
          this.currentPage = response.data.meta?.current_page || 1;
          this.total = response.data.meta?.total || 0;
        }
      } catch (error) {
        console.error('Error fetching rooms:', error.response?.data || error);
        this.rooms = [];
        this.currentPage = 1;
        this.total = 0;
      }
    },

    /* -------------------------------------------------------------------
     * CREATE ROOM
     * ------------------------------------------------------------------- */
    async store(payload) {
      try {
        const response = await axios.post('/rooms', payload);
        if (response?.status === 201) return true;
      } catch (error) {
        console.error('Error creating room:', error.response?.data || error);
      }
      return false;
    },

    /* -------------------------------------------------------------------
     * UPDATE ROOM
     * ------------------------------------------------------------------- */
    async update(id, payload) {
      try {
        const response = await axios.post(`/rooms/${id}`, {
          ...payload,
          _method: 'PUT'
        });
        if (response?.status === 200) return true;
      } catch (error) {
        console.error('Error updating room:', error.response?.data || error);
      }
      return false;
    },

    /* -------------------------------------------------------------------
     * DELETE ROOM
     * ------------------------------------------------------------------- */
    async destroy() {
      if (!this.id) return false;
      try {
        const response = await axios.delete(`/rooms/${this.id}`);
        if (response?.status === 204 || response?.status === 200) {
          this.id = null;
          return true;
        }
      } catch (error) {
        console.error('Error deleting room:', error.response?.data || error);
      }
      return false;
    },

    /* -------------------------------------------------------------------
     * EXPORT ROOMS
     * ------------------------------------------------------------------- */
    async export() {
      try {
        const url = `${axios.defaults.baseURL}/rooms/export?type=${this.exportType}`;
        if (this.exportType === 'print') {
          window.open(url, '_blank');
        } else {
          window.location.href = url;
        }
        return true;
      } catch (error) {
        console.error('Error exporting rooms:', error);
      }
      return false;
    }
  }
})
